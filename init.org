...
For this reason I rebind the =<escape>= keys for the evil command line here. The other interfaces are managed in ther respective subtrees.
...
However an =<escape>= press when already in normal mode should always allow me to exit the minibuffer.
...
Implementation of =my/operate-on-number=.

#+begin_src elisp

(defun my/operate-on-number (fun count)
  (setq old-point (point))
  (skip-chars-backward "1-9")
  (or (looking-at "[0-9]+")
      (user-error "No number at point"))
  (let* ((old-string (match-string 0))
         (result (apply fun `(,(string-to-number old-string) ,count)))
         (new-string (format "%s" result)))
    (replace-match new-string)
    (goto-char old-point)
    (backward-char (- (length old-string)
                      (length new-string)))))

(defun my/operate-on-number-key (&optional arg)
  (interactive "P")
  (let* ((key-vec (this-command-keys-vector))
         (key-str (key-description key-vec))
         (key (car (last (s-slice-at "." key-str)))))
    (my/operate-on-number (intern-soft key)
                          (if arg arg 1))))

(defmacro my/operate-on-number-generate-operation (fun default-arg)
  (let ((operation
         `(lambda (&optional arg)
            (interactive "P")
            (my/operate-on-number ',(eval fun)
                                  (if arg
                                      arg
                                    ,default-arg)))))
    `(identity ,operation)))
...
I bind several operations to keys under the =C-.= prefix.
...
(general-define-key
 :states '(normal visual insert emacs)
 :prefix "C-."
 "+" 'my/operate-on-number-key
 "-" 'my/operate-on-number-key
 "*" (my/operate-on-number-generate-operation '* 2)
 "/" (my/operate-on-number-generate-operation '/ 2)
 "^" (my/operate-on-number-generate-operation 'exp 2)
 "<" (my/operate-on-number-generate-operation 'ash 1)
 ">" (my/operate-on-number-generate-operation (lambda (val arg) (ash val (- arg))) 1)
 "%" (my/operate-on-number-generate-operation '% 2))
...
Using this code requires the following directory structure inside your =user-emacs-directory=:
+ direnv
  + active
    .envrc 
    default.nix
...
(defvar ad-hoc-direnv-active-directory
  (f-join user-emacs-directory "direnv" "active"))

(defvar ad-hoc-direnv-inactive-directory
  (f-join user-emacs-directory "direnv" "inactive"))
...
(defun ad-hoc-env-exit ()
  ...
  (unless (bound-and-true-p ad-hoc-in-env)
    (user-error "Not in ad-hoc environment"))
...
Flyspell can be very slow at times. These settings supposedly improve its performance at times.
...
As of right now Nim integration is lacking in Nix/Nixpkgs, so I have ntangle installed in my home directory under =~/.nimble/bin/ntangle=. [TODO: create proper nix pkg description]
...
(defun my/org-ntangle-maybe-tangle-file (file)
  (let ((bin (f-join (getenv "HOME") ".nimble/bin/ntangle")))
    (if (f-exists-p bin)
        (start-process
         ;; name + buffer + cmd
         "org-ntangle-tangle" "*org-ntangle-process*" bin
         ;; arguments
         file)
      nil)))
