...
(defmacro key-fun (action &rest args)
  (let* ((hint (concat (symbol-name action) "-"
...
(defmacro key-defun (name &rest body)
  (let* ((hint (symbol-name name)))
    `(list (lambda () (interactive) ,@body) :wk ,hint)))
...
(defun nuke-buffers ()
  "Kill all emacs buffers but current and internal, but only after saving \*scratch\*"
  (interactive)
  (when (get-buffer "*scratch*")
    (persistent-scratch-save))
  (--each (-difference
           (--filter
            (not (s-prefix? " " (buffer-name it)))
            (buffer-list))
           `(,(current-buffer)))
    (kill-buffer it))
...
(defun nuke-emacs ()
  "Kill all buffers and emacs, but only after saving \*scratch\*"
  (interactive)
  (nuke-buffers)
...
Also bind =j+k= to go from insert to normal state through key-chord.
...
.
**** Amx
Amx is an alternative to the no longer maintained smex.
...
** Planned
*** TODO rework organization arround first headings
<2019-03-27 Wed>
*** TODO yasnippet
<2019-03-27 Wed>
