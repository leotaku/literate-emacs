...
As of right now Nim integration is lacking in Nix/Nixpkgs, so I have ntangle installed in my home directory under =~/.nimble/bin/ntangle=. [TODO: make proper nix pkg description]
...
(defun my/org-ntangle-maybe-tangle-file (file)
  (let ((bin (f-join (getenv "HOME") ".nimble/bin/ntangle")))
    (if (f-exists-p bin)
        (start-process
         ;; name + buffer + cmd
         "org-async-tangle" "*org-ntangle-process*" bin
         ;; arguments
         file)
      nil)))
