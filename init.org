...
**** persist
Persist is an emacs-lisp library that aims to provide a simple and reliable interface to session-persistent variables.
It is written by me and not yet ready for production, however I use it in my personal configuration.

#+begin_src elisp :tangle no
(use-package persist
  :straight (persist :type git :host github
                     :repo "leotaku/persist.el"))
#+end_src
...
Disallow recursive minibuffers.
They are unfortunately required for some specific edge-cases, but can sometimes cause weird glitches.
#+begin_src elisp
...
(use-config desktop
  ...
  (add-hook 'desktop-after-read-hook 'my/restore-font))
#+end_src
...
(defun my/restore-font (&rest _)
  (let ((font (format "%s-%d" my/current-font my/current-font-size)))
...
(defun my/advice-evil-normal-state (fun &optional arg)
  (if (and mark-active (evil-insert-state-p))
      (deactivate-mark)
    (apply fun (list arg))))
#+end_src
...
.
****** Vimish-fold
The evil-vimish-fold package offers evil bindings for vimish fold.
I enable them here.
...
***** Terminal
Evil doesn't really have support for Emacs in terminal by default, so I fix it up myself.

#+begin_src elisp
(general-define-key
 :states '(insert normal visual)
 "TAB" (general-key "<tab>")
 "S-TAB" (general-key "<backtab>"))
#+end_src

****** TODO Terminal-cursor-changer
**** Ivy
...
Aggressive-indent-mode currently overrides this keybinding.
...
*** TODO Folding/Vimish
I use vimish-fold for classical buffer folding.
...
Bind =<C-tab>= in folded regions to toggle.
...
Misc company setting, mostly related to frontend display and keybindings.
...
(general-define-key
 ...
 "<return>" nil
 ";" 'company-complete
 ;; FIXME: not agnostic, needs better solution
 "<tab>" (key-defun yas-or-company
           (or (yas-expand)
               (company-select-next)))
...
I prefer the company-postframe frontend, as it doesn't mess with buffer text properties and seems to otherwise be equivalent to the default frontend.
...
** Languages
*** All Lisps
Lisp-like languages all feature a very similar paren-based structure and coding style. Most lisps are also well-suited for an interactive evaluation-oriented workglow.
For this reason it makes sense to configure some generic options for all lisp-like languages.

Lispy is a package that provides an improved Lisp editing experience through state-based navigation, with a special focus on Emacs Lisp and Clojure.
I also hook aggressive-indent-mode into lispy, as I generally want it enabled for all lisp dialects.

Relevant dialects may enable lispy in their own subtrees.
...
Lispyville provides vim/evil-compatible bindings for lispy.
...
These are my personal lispy additions, mostly for better evil integration.
They are still experimental and probably clash with some of abo-abos design choices for lispy.

#+begin_src elisp

(defun my/lispy-up-insert ()
  (interactive)
  (let ((old-point (point)))
    (lispy-left 1)
...
(general-define-key
 ...
 :states '(insert emacs)
 "<tab>" 'my/lispy-step
 "<backtab>" 'my/lispy-step-backward)

(general-define-key
 :definer 'minor-mode
 :keymaps 'lispy-mode
 :states '(normal visual insert emacs)
 "C-l" 'my/lispy-up-insert)
...
(use-package tex
  ...
  :defer t
  :hook
  ((TeX-mode LaTeX-mode) . visual-line-mode)
  ((TeX-mode LaTeX-mode) . aggressive-indent-mode)
  :config
...
.
*** Helpful
Helpful is a prettier interface to the Emacs help system.
...
Rebind the default help bindings to helpful equivalents.
...
I find that message-mode buffers are usually hard to edit, requiring many keystrokes to compose a simple message.
However this can be helped by "wrapping" any message buffer using ivy for completions and yasnippet for quickly jumping through the template.

This setup currently only works with notmuch, but could be easily generalized further. [TODO]

#+begin_src elisp

(defun my/message-wrap-buffer (&rest _)
  (let ((yas-snippet (my/message-generate-yas-for-buffer)))
    (erase-buffer)
    (goto-line 0)
    (yas-expand-snippet yas-snippet)
    (evil-insert 1)
    (setq-local my/message-buffer-wrapped t)))

(defun my/message-generate-yas-for-buffer ()
  (concat
   (s-join "\n"
           (--map-indexed
            (if (s-matches? "^\\w*:" it)
                (let* ((line (s-split-up-to ":" it 1))
                       (key (car line))
                       (value (s-trim (cadr line))))
                  (concat key ": "
                          (my/message-generate-key-function key value)))
              it)
            (s-split "\n" (buffer-string))))
   "$0"))

(defun my/message-generate-key-function (key value)
  (cond
   ((and (string= key "To") (string= value ""))
    (ivy-read "To: " (hash-table-keys notmuch-address-completions)
              :initial-input value))
   ((string= key "Subject")
    (concat "${1:" value "}"))
   (t
...
(general-define-key
 ...
 "<tab>" (key-fun my/message-wrap-buffer))
...
**** Notmuch
Notmuch is a mature maildir indexing program with several frontends.
The Emacs frontend is also called Notmuch.
...
(use-package notmuch
  ...
  :config
  ;; Initialize address map (async)
  (notmuch-address-harvest))
#+end_src

I bind =notmuch-search= with =tag:unread= to =C-x m= for easy access.

#+begin_src elisp
(general-define-key
 "C-x m" (key-fun notmuch-search "tag:unread"))
#+end_src

Remove the default message-inherited hooks from =notmuch-mua-send-hook=, as they otherwise get run twice when sending.

#+begin_src elisp
(remove-hook 'notmuch-mua-send-hook 'notmuch-mua-message-send-hook)
#+end_src

Notmuch-search by default does not immediately show when a message has been read, fix that.

#+begin_src elisp
(advice-add 'notmuch-search-show-thread :before 'my/advice-notmuch-read)
(defun my/advice-notmuch-read (&rest arglist)
  (when (-contains? (notmuch-search-get-tags)
                    "unread")
    (notmuch-search-tag
     (list "-unread"))))
#+end_src

It also strangely highlights unread messages, which I dislike and thus turn off.

#+begin_src elisp
(setq notmuch-search-line-faces (list))
#+end_src
...
The default keybindings for =<return>= and =<C-return>= only really work for outline headings. I bind the indentation-aware return to =<return>= for better list support.
...
**** TODO Agenda
I bind the default org-agenda menu to =C-x a= for easy access.
...
Use the =org-default-notes-file= as the initial source for org-agenda entries.

#+begin_src elisp
(use-config org-agenda
...
**** Habit
Org-habit is a org module that provides tracking of recurring tasks (habits) for org.
Unfortunately the builtin org-habit does not support the sexp date specification format, meaning recurring eg. on specific days of the week is impossible.

Org-habit-plus fixes the problem of recurring on weekdays by defining a new =:HABIT_WEEKDAYS:= property, which can then be used in org-habit dynamic subtrees.
...
(use-package org-habit-plus
  ...
  :after org
  :config
  ;; org-habit-plus doesn't set org-habit-scheduled-past-days
  (setq org-habit-scheduled-past-days org-scheduled-past-days))
#+end_src
...
(general-define-key
 ...
 "C-h" help-map
 "M-x" 'counsel-M-x
 "<escape>" (key-fun term-send-raw-string "\e[[m"))
...
* Changelog                                                        :noexport:
** Planned [18/45]
*** TODO unifiy terminal configurations
*** TODO helpful
*** TODO notmuch tree multiple buffers
*** TODO org space after headline insert
[[file:straight/repos/org/lisp/org.el::(defun org-insert-heading (&optional arg invisible-ok top)]]
*** TODO learn about org-store-link
*** TODO find buffer group solution
*** TODO fix outline not skipping fold
*** TODO find a better way for in-buffer capture templates
*** MAYBE investigate if evil works with multi-instances
*** TODO use delete-indentation in smart-backspace
*** DONE move lispy and aggressive indent out of [[Emacs Lisp]]
*** DONE BUT learn lispy and bind tab + C-l
...
*** DONE BUT org-mode things
habit, schedule, agenda, clocking
*** DONE keyfreq-mode
*** DONE ediff
*** CANCELED debug-on-error as default
...
*** MAYBE incorporate prescient.el
...
** Variables                                                       :noexport:
# Local Variables:
# eval: (add-to-list 'org-capture-templates `("l" "Literate Emacs Task" entry (file+headline ,(buffer-file-name) "Planned") "* TODO %^{Task}" :immediate-finish t :prepend t))
# End:
